...
4. **Registers** `new@eventy.com` via `/api/auth/register` (idempotent if registration was already run).
5. **Builds** the **React** app with `REACT_APP_API_URL=http://127.0.0.1:4000`, then serves the **`build/`** folder on port **3000** with `serve`.
6. Runs **`npx playwright test --project chromium`** from `qa/` (Chromium only by default to keep CI fast and stable).
